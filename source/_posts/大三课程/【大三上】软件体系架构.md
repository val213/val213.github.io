...
2. Please offer a reference model, architecture style and reference architecture for
the system (Score 10). / 请为系统提供参考模型、架构风格和参考架构（得分10）。

3. Please explain which quality attributes the reference architecture achieved and
which tactics are chosen (Score 10). / 请解释参考架构实现了哪些质量属性以及选择了哪些策略（得分10）。

4. Please present and explain the decomposition view, concurrency view and
deployment view of the system design (Score 10). / 请介绍和解释系统设计的分解视图、并发视图和部署视图（得分10）。
