...
![alt text](`O83B$FH[BN@Q7M[Y`7Q]@M.png)
### 4+1视图
...
![alt text]({DBDUOVW{Y%ONIA7$D~KOUG.jpg)
### 九大核心工作流
...
>![alt text](85285b999c78e9292dd729325718700.png)
...
|---|---|---|
|关联|参与者与用例之间的关系|
|泛化|参与者之间或用例之间的关系|![alt text](image-140.png)|
|包含|用例之间的关系，类似has-a，被包含的用例是一个大用例中的小步骤|![alt text](image-139.png)|
|扩展|用例之间的关系，类似继承，从一个基础用例扩展而来，有额外的触发条件以及其他细节|![alt text](image-138.png)|
...
- 各种关系的强弱顺序： 泛化 = 实现 > 组合 > 聚合 > 关联 > 依赖
    ...
    ![alt text](image-133.png)
        - 在边边上表示这个关联的名称
        - ![alt text](<@]U)F8~8$X%%XL0J84PZ~}0.jpg>)
...
#### 接口
![alt text](a4d35850f29b4d507ce49f39fa63281.jpg)
...
- 聚合：has-a，整体与部分之间的关系，部分可以离开整体单独存在
    ...
    ![alt text]([WGL3@0{}76IT3%@CYQNF_E.png)
- 组合：contains-a，整体与部分之间的关系，部分不能离开整体单独存在，但是组合关系更强，部分和整体的生命周期一样

![alt text](GOVT%5IPH49LV%$8S_MIE02.jpg)
选A。
#### 活动类
活动对象的类为活动类。用双竖线表示。
...
|:---:|:---:|---|
|alt	|Alternative fragment for mutual exclusion; conditional logic expressed in the guards 可选的互斥选项 类似if-else|![alt text](image-159.png)|
|loop	|Loop fragment while guard is true; can also write loop(n) to indicate looping n times 循环 类似for|![alt text](image-160.png)|
|opt	|Optional fragment that executes if guard is true 可以选择的选项，例如开发票，可以执行也可以不执行，而alt必须执行一个|![alt text](image-158.png)|
...
![alt text](<7IU$4$QSGSV3WDZJXSBL(15.jpg>)选C
#### operation contract
...
uml2.0：
![alt text](288f0760a23f8a6092eab4d8f20a053.jpg)
...
#### 信息系统逻辑架构中常见的层
松散分层耦合
![alt text](f2622a3cd98ef2d260b8a15407baf7c.jpg)
...
### 1、领域模型
不写方法，区别于类图
![alt text](IH%]BLD$6CO7V2XUVRQ874A.jpg)
...
#### 用例实现
用例实现就是设计模型
![alt text](faff094d9258c153b91dbfe33a73ead.png)
![alt text](fcc89a708152a7954fa1768fe83a044.png)
### 4、操作模型
### 5、描述结构模型
### 6、用例分析模型
### 7、类中的职责分配
